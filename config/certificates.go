...
func loadCertificates(config *EntitlementsConfig) error {
	...
	if err != nil {
		return fmt.Errorf("unable to load certificate-key pair from the test files: %w", err)
	}

	err = loadCAFileIntoSystemCAsBundle(config, caCertFilePath)
	if err != nil {
		return fmt.Errorf("unable to load the CA certificate chain from the test files: %w", err)
	}
...
func loadCAFileIntoSystemCAsBundle(config *EntitlementsConfig, caFilePath string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to append %s to RootCAs: %w", caFilePath, err)
	}
...
func loadCertificateKeyPairFromFile(config *EntitlementsConfig, certificateFilePath, keyFilePath string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to load the certificate-key pairs from file: %s", err)
	}
